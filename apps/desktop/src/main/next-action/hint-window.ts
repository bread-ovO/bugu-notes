...
import type { NativeNextObserver } from './native-input'
...
export class NextActionHintWindow {
  ...
  private nativeDeadline = 0
  private nativeLease = false
  private loaded = false
  private readyResolve: (() => void) | null = null
  private disposed = false
  private inputState: InputActivity = {
    known: false,
    composing: false,
    lastInputAt: 0,
  }
  ...
  constructor(
    private options: {
      pageURL: string
      preload: string
      registry: NextTargetRegistry
      native?: Pick<NativeNextObserver, 'capability' | 'arm' | 'release'>
  ...
  private trusted(event: IpcMainEvent): boolean {
    return isTrustedSender(
      event,
      this.window?.webContents ?? null,
      this.options.pageURL,
    )
  }
  private readyMessage = (event: IpcMainEvent) => {
    if (this.trusted(event)) this.readyResolve?.()
  }
  private confirmMessage = (event: IpcMainEvent, id: unknown) => {
    if (
      this.trusted(event) &&
      typeof id === 'string' &&
      id === this.current?.context.id
    )
      void this.confirm()
  }
  ...
  private release(): void {
    this.options.native?.release()
    this.nativeLease = false
    if (this.lease) {
      globalShortcut.unregister(this.lease)
      this.lease = null
    }
  }
  async present(
    ...
    if (
      ...
      this.exitTimer ||
      !isCurrent() ||
      presentation.target.id !== presentation.context.targetId ||
      presentation.target.revision !== presentation.context.targetRevision
    )
      return false
    if (
      presentation.context.shortcut === 'Tab' &&
      !this.options.native?.capability.tab
    )
      return false
    ...
    if (!this.window) {
      ...
      this.window.webContents.on('will-navigate', (event, url) => {
        if (!isTrustedPage(url, this.options.pageURL)) event.preventDefault()
      })
      ...
      const rendererReady = new Promise<void>((resolve, reject) => {
        ...
        readyTimer = setTimeout(
          () => reject(Error('NEXT_RENDERER_UNAVAILABLE')),
          5000,
        )
      })
      try {
        await Promise.all([
          this.window.loadURL(this.options.pageURL),
          rendererReady,
        ])
        this.loaded = true
      } catch {
    ...
    if (this.disposed || !this.loaded || !this.current || !this.valid()) {
      this.dismiss()
      return false
    }
  ...
  private async tick(): Promise<void> {
    ...
    if (!state || !this.current || !this.window || !this.valid()) {
      this.dismiss()
      return
    }
    state.tick(this.now())
    ...
    if (state.phase !== 'visible') {
      ...
      if (shortcut === 'Tab' && !this.nativeLease) {
        if (
          !this.inputState.known ||
          this.inputState.composing ||
          this.now() - this.inputState.lastInputAt < 1200
        )
          return
        this.arming = true
        const current = this.current
        const until = Math.min(
          this.now() + state.context.durationMs,
          state.context.validUntil,
        )
        const armed = await this.options.native!.arm(
          state.context.id,
          until,
          (id) => {
            if (this.current?.context.id === id) void this.confirm()
          },
        )
        this.arming = false
        if (!armed || this.current !== current || !this.valid()) {
          this.options.native?.release()
          return
        }
        this.nativeLease = true
        this.nativeDeadline = until
      }
      if (shortcut !== 'click' && shortcut !== 'Tab' && !this.lease) {
        ...
        if (!globalShortcut.register(shortcut, () => void this.confirm()))
          return
        this.lease = shortcut
      }
      if (
        !state.ready(
          this.now(),
          this.inputState,
          shortcut === 'click' || this.nativeLease || this.lease !== null,
        )
      ) {
        this.release()
        return
      ...
      if (this.nativeLease)
        state.deadline = Math.min(state.deadline, this.nativeDeadline)
      this.window.webContents.send('next-hint:show', {
        id: state.context.id,
        label:
          this.current.target.kind === 'open-app'
            ? `打开 ${this.current.target.label}`
            : this.current.target.label,
        keycap: shortcut === 'click' ? null : shortcut,
        durationMs: state.deadline - this.now(),
      })
  ...
  input(activity: InputActivity): void {
    const changed = activity.lastInputAt > this.inputState.lastInputAt
    this.inputState = { ...activity }
    if (this.current?.context.shortcut === 'Tab' && (changed || !activity.known || activity.composing)) {
      this.release()
      this.state?.input(this.now())
      this.window?.hide()
      if (this.state?.phase === 'dismissed') this.dismiss()
    } else if (!activity.known || activity.composing) {
      this.release()
      this.dismiss()
    }
  }
  ...
  dismiss(): void {
    const id = this.current?.context.id
    this.state?.dismiss()
    this.release()
    this.hide()
    if (id) this.options.onDismiss?.(id)
  }
  private hide(): void {
