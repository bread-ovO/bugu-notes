...
void observeVisible(){
 ...
 std::map<long long,double> since;std::set<long long> emitted;
 while(true){Sleep(1000);std::vector<Candidate> active;HWND window;{std::lock_guard<std::mutex> lock(guard);window=GetForegroundWindow();for(auto c:candidates)if(c.app==appIdentity)active.push_back(c);}
  if(active.empty()){since.clear();emitted.clear();{std::lock_guard<std::mutex> lock(guard);send("{\"type\":\"visible-set\",\"ids\":[]}");}continue;}
  IUIAutomationElement* root=nullptr;if(FAILED(uia->ElementFromHandle(window,&root))||!root)continue;
  std::vector<IUIAutomationElement*> stack{root};std::set<long long> found;int count=0;
  while(!stack.empty()&&count++<400){auto node=stack.back();stack.pop_back();BOOL offscreen=TRUE,password=TRUE;CONTROLTYPEID type=0;node->get_CurrentIsOffscreen(&offscreen);node->get_CurrentIsPassword(&password);node->get_CurrentControlType(&type);
  ...
  std::lock_guard<std::mutex> lock(guard);if(window!=GetForegroundWindow())continue;
