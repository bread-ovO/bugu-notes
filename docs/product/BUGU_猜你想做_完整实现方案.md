...
状态：**方案已确认，实施中；内部集成尚未达到完整交付门禁**。创建：2026-09-18。更新：2026-09-19。代码核对基线：`upstream/main@8b0d594`。对应 [Issue #158：feature: 猜你想做](https://github.com/bread-ovO/bugu-notes/issues/158)。
...
**用户遇到一类工作事件时，不咕在屏幕左上角短暂提示可能想去的工具；按设定的确认键（默认 Tab）即可跳转，不操作则倒计时结束后滑走。** 本功能独立于桌宠，使用越久，建议越符合本人处理不同事情的习惯。
...
5. 独立左上角短提示、可自定义的确认键（默认 Tab）、输入时延后展示、边框倒计时与退场动画，以及错过后主动查看建议的入口。
...
3. **确认模型与提示方式**：显示实际模型服务、拟发送的字段，以及“左上角提示 3 秒、提示期间按确认键跳转”的行为。确认键默认 Tab，可当场修改；使用 Tab 时，输入中暂缓展示，停下后再显示。明确这是临时系统级按键响应，用户可关闭主动提示或关闭学习。完成后进入“学习中”，先积累实际工具选择，不由模型凭空给首个主动推荐，也不注入演示任务。
...
用户不需要打开跟进清单或桌宠。模型给出有效建议后，在当前工作屏幕的可用区域左上角显示一条首选；自定义确认键（默认 Tab）或点击确认。错过后可通过 BUGU 顶栏或手动快捷键查看仍有效的建议及其他候选。没有可靠上下文时不弹猜测性空卡；主动查看时允许用户选择授权对象。
...
**用户已确认：所有目标软件采用同一套跳转规则，不对 Codex 单独特殊处理。** 能找到当前事件对应的会话、页面、文档或项目，且该软件支持经过验证的精确入口时，直接打开；否则打开已登记、可验证的应用或服务入口，并准确显示“打开某应用”。用户自行决定选择或新建内容，本功能不擅自创建 AI 会话、文档、任务，不发送消息，也不启动 Agent 执行工作。

| 目标软件示例 | 有精确目标且能力已验证 | 暂无精确目标 |
| --- | --- | --- |
| Codex / Claude Code / Kimi 等 AI 工具 | 进入当前项目对应会话 | 打开已登记的应用入口；仅 CLI 的工具可定位已登记的终端/项目入口，不为跳转自动运行 Agent 命令 |
| 飞书及其他已接入通讯软件 | 打开对应讨论或消息 | 打开该通讯应用，不随意选一个聊天或发送内容 |
| GitHub / 浏览器 | 打开已验证的 PR、Issue 或相关页面 | 打开配置好的服务/应用入口，不创建 PR 或 Issue，不猜目标网址 |
| 文档软件 / 编辑器 | 打开有权限且存在的对应文档或项目 | 打开应用，不创建文件或修改内容 |

这些例子说明适配器应实现的统一行为，不表示当前主分支已具备所有软件的集成。只展示已经注册并验证可执行的目标；没有安装或可用入口时，不能显示一个按了没有效果的建议。为新软件增加适配器或用户确认的应用入口即可复用学习、短提示、自定义确认键和纠错流程，无须复制一套推荐逻辑。
...
**用户确定的交互：左上角弹出一个小弹窗，展示两三秒，提示可能想去的工具；确认键可自定义，默认 Tab，边框有轻量倒计时，时间到就滑走。选择 Tab 时，正在输入先不弹，停止输入后再弹。** 默认展示 3 秒，允许用户改为 2 秒；需更长阅读时间的用户可选择延长或手动关闭，不强制所有人使用短时限。
...
默认宽 360 px、高 64 px、圆角 12 px、内边距 12 px，图标 20 px。只有应用图标、一句话和当前实际确认键的键帽（默认 Tab）；不展示候选列表、长解释或数字倒计时。示意：
...
默认一句“可能你想去 Codex”；能精确定位时可显示“继续登录修复 · Codex”，但必须有对应能力。卡片单次显示期间固定事件、目标、建议修订和文案，不在用户准备按确认键时更换目标。新建议不堆叠、不刷新倒计时；原上下文改变或建议失效则提前退场。
...
只有建议已经有效、窗口可见且本次确认通路已就绪时（键盘模式需按键租约就绪，仅点击模式需点击确认就绪），才开始 3 秒倒计时。模型在后台尚未返回时不弹加载卡，也不提前消耗可操作时间。视觉进度与按键有效期来自同一单调时钟截止时间；到点先撤销按键响应，再播放退出动画，退出期间不能跳转。动画卡顿不能延长原生按键有效期。

设定的确认键或点击确认后，原子消费本次建议的确认令牌，立即解除按键响应并收起卡片，再由可信执行器复核目标并跳转。确认失败不能继续执行旧目标；用户可在手动查看入口看到原因。没操作而自动消失只记为“未选择”，不作为讨厌该工具的负反馈。

### 6.3 自定义确认键与输入避让（用户已确认）

设置 → 猜你想做 → **跳转确认键**，默认 Tab，支持录制自定义组合键、恢复默认，也可关闭键盘确认而保留点击。这个键只在短提示有效期间确认跳转，与“主动打开次级面板”的快捷键分别配置。卡片键帽必须实时反映实际生效的配置，不能改了键仍显示 Tab。

支持裸 Tab，以及平台允许的修饰键组合；不把普通字母、数字或裸回车配置成全局单键确认。保存前检查系统保留组合、BUGU 自身快捷键与当前平台可注册能力；注册失败时保留原配置并说明原因，不偷偷换回 Tab。不能保证检测出所有第三方软件的应用内快捷键，因此提供用户主动发起的测试提示。修改配置时先解除旧租约、隐藏当前卡片，再使用新配置；不得同时响应两个键。

用户仍停留在飞书等来源应用时，看到弹窗即可按所设确认键跳转，无需先点击或聚焦弹窗。主进程拥有临时按键租约，绑定 `suggestionId / targetRevision / deadline / sourceContextVersion / shortcutRevision`。macOS 评估带必要权限的原生事件 tap，Windows 用专用原生线程的键盘 hook，X11 验证临时键位抓取；修饰键组合或其他 Electron 快捷键方案只在实测可可靠注册、释放和阻止重复传递的平台采用。不能只给 renderer 添加 `keydown` 就声称系统级确认已可用。[Apple CGEvent tap](https://developer.apple.com/documentation/coregraphics/cgevent/tapcreate(tap:place:options:eventsofinterest:callback:userinfo:))、[Microsoft LowLevelKeyboardProc](https://learn.microsoft.com/en-us/windows/win32/winmsg/lowlevelkeyboardproc)。

**当确认键为 Tab 时，先等用户停止输入。** 建议就绪后若仍有输入活动或输入法正在组词，只保留一个待展示候选，不显示卡片、不开始倒计时、不截获 Tab。初始工程参数为最后一次输入活动后连续安静 1,200 ms，且输入法已结束组合，再重新校验事件、目标与权限；仍适用才弹出并开始完整 3 秒。1,200 ms 仅控制界面避让，不参与事件关联或习惯学习，可依据真实输入法测试调优。

只获取“正在输入/最后活动时刻/组合中/未知”等最小状态，不收集文字、具体输入内容或历史键码；状态只在本机内存使用，不进入习惯库或模型请求。原生或受支持应用适配器无法可靠判断时，不武断认定用户已经停下：不启用裸 Tab 自动提示，设置中说明当前能力，仍可通过手动入口、点击或已验证的其他组合键使用功能。

如果 Tab 提示显示后用户又开始输入，先释放 Tab、收起提示，再等待其停下；该次输入完整传给原应用。因输入而中断不记为拒绝或一次完整曝光；原建议可在重新验证后恢复一次完整 3 秒，连续中断则留在手动入口，避免反复闪现。新的普通 Tab 在安全租约内才算确认；输入法组合期间的 Tab 必须留给原应用。输入状态变化与按键确认在原生处理端按顺序检查，不能等 UI 收到异步消息后才解除按键。

选择其他已验证的修饰键组合时，不强制套用 Tab 的停止输入等待条件；仍不抢焦点，遵守平台输入法、系统保留键和免打扰约束，卡片显示真实组合键。可独立选择仅点击确认，此时不安装确认按键租约。

按键路径只处理配置的确认组合及其配对按键状态，不记录输入文字，不保存或上传其他键码。有效确认不应同时传到原应用造成额外动作；持续按住、自动重复、双击不重复打开。配置为裸 Tab 时，Shift+Tab、Ctrl/Command+Tab、Alt+Tab 保持原用途；出现前已按住的确认键不能自动触发。窗口消失、授权撤销、上下文失效、改键、锁屏或进程异常都释放租约，helper 自带硬截止保护，主进程失联也不能长期占键。

原生 Wayland 等环境未必允许自由定位桌面窗口或接管裸 Tab；必须分开探测“左上角定位”“非激活显示”“临时 Tab”三种能力。不能承诺所有合成器都支持。支持矩阵如实列出受测环境；能力不具备时展示可点击入口及真实可用快捷键，禁止继续绘制不可用的 Tab 键帽，也不自动抢焦点绕过限制；仅点击模式不依赖键盘能力。标准 BrowserWindow 的非激活显示和 Wayland 定位限制需按项目所用 Electron 版本实测。[Electron BrowserWindow](https://www.electronjs.org/docs/latest/api/browser-window)。

### 6.4 事件触发、错过与空状态

启用“猜你想做”并确认上述交互后，短提示是默认主入口；用户可随时关闭主动提示而保留手动查看。只在该类事件已完成真实选择积累、有明确事件、高置信且目标可执行时显示，同一事件＋动作＋目标修订默认只完成一次展示，输入中断的有限恢复按第 6.3 节处理；模型重试、窗口来回切换、重复同步不重弹。不同事件可各自产生提示，但不堆积，不沿用桌宠的“每小时两次/每天六次”话语配额；突发事件合并挑选当前最相关的一条，不能连续排队弹出多张卡。
...
次级面板承担查看来源、其他目标和纠错。未启用、未授权、无上下文、正在分析、离线、预算耗尽或目标失效等状态只在用户主动查看时说明，不反复弹左上角错误卡。学习中或没有可靠建议时可显示明确标注的“固定入口/最近打开”，不冒充 AI 推荐。用户主动打开并聚焦次级面板后使用标准 Tab 导航、方向键和 Enter，不延续桌面短提示的确认键租约。

### 6.5 推荐错了怎么纠正（用户已确认）

默认不在每次跳转后追问“对不对”。短提示通过右键或 hover 后的更多按钮提供纠错；用户主动打开纠错面板时，立即解除确认键和倒计时，后续操作只作用于该条建议。BUGU 顶栏入口和次级面板另设“最近建议”，保留最近 20 条可纠错记录（最长 30 天）；卡片消失或已经跳走，也能找回刚才那条。

| 用户遇到的情况 | 界面与处理 | 如何影响学习 |
| --- | --- | --- |
| 没看见、不想现在处理，等它消失或关闭 | 安静收起，不自动追问 | 不扣分、不记错 |
| 这一次不该去 Codex | 选择“这次不对”，本事件不再推荐该动作；可选正确工具，也可直接结束纠错 | 撤销本事件的错误关联/正向贡献，不据此拉黑 Codex 或否定所有同类偏好 |
| 这类事通常想去 Claude Code | 选择“这类事用…”并确认事件类型、范围和工具 | 保存可撤销的类型级明确偏好，立即重排同类建议；不改其他类型或其他项目的内容关系 |
| 事件被理解错了，比如本来只想了解情况而不是修复 | 选择“事情理解错了”，简述或改选类型 | 先纠正分类，再重算关联与贡献，不把分类错误当工具不受欢迎 |
| Codex 对，但打开的会话不对 | 选择“会话不对”，可重新选目标 | 只纠正具体目标层，保留有依据的工具层选择 |
| 应用打不开或链接失效 | 明确显示打开失败，并允许重试/选择其他真实入口 | 归为执行故障，不当作用户偏好反馈 |

若用户忽略 Codex 建议、自己去了 Claude Code，只在确认仍处理同一事件时，记下 Claude Code 的真实选择；不能仅凭换了应用就断言 Codex 推荐错误。快速返回也不是明确差评。用户已按键跳错时，在最近建议中可选择“回到来源”；仅打开已验证的原应用/来源对象，不自动把用户拉回，不声称撤销外部应用里的工作。

纠错立即使相关建议和缓存失效，解除尚存的按键租约；同一错误建议不能紧接着重弹。支持撤销上次纠错，保留前后版本并原子替换学习贡献，防止一纠错就双倍计数。涉及的来源已撤权/删除时不恢复旧内容。用户已确认上述纠错方式：平时不追问，最近建议可回找，按本次/类型/具体目标分层纠正，并支持撤销。该确认仅冻结产品交互，不代表代码已经实现。
...
| 主动提示与确认键 | 独立左上角短提示，显示期间响应自定义键；Tab 模式在停止输入后才弹 | 功能首次关闭；向导确认后随功能启用，可独立关闭 |
...
3. **关联事情**：先查明确对象、事项关联与用户确认映射；需要语义判断时，模型结合已授权来源消息及必要上下文、目标会话/页面的相关内容，判断此次切换对应哪件事，并返回双方证据或“不确定”。不确定结果暂不计入习惯。用户临时去其他应用不会结束原事件；切到同一个工具也不自动证明是在处理它。
...
### 9.4 双边内容辅助关联（2026-09-19 用户已确认）

关联模型可使用**已授权来源消息和目标会话中的相关内容**，判断真实应用切换是否属于同一事件。例如，飞书反馈某项目登录报错，目标编码会话正在处理同一项目的对应登录问题，可作为关联候选；目标会话在处理另一个项目，即使也出现“登录”也不能据此归入。

每次输入只取候选事件涉及的来源片段、必要上下文、目标侧相关片段和当前有效引用，合计遵守第 10 节的 6,000 字符/20 条证据上限。不发送两个应用的全部历史，不扩大连接授权。来源侧、目标侧和所选模型发送用途必须分别有效；目标应用可激活不代表其内容已授权可读。未接入目标内容、权限不足或片段不足时，不通过读屏、剪贴板或输入记录偷偷补齐；缺少其他明确依据就记为未归因，暂不学习。

`attribute-transition` 返回已有业务事件 ID、真实切换 ID、关联/无关/不确定状态、双方证据引用及修订。领域层核验引用确实存在、所属项目/账号与授权一致、没有对象冲突；内容矛盾、候选并列、模型超时、未知 ID 或引用失效时，结果不进入偏好贡献。模型自报高置信不能覆盖这些校验；引用有效也不等于已经证明因果，通过校准与依据检查的自动关联仍按弱样本计权，可由用户纠正。

来源或目标新增有用内容后，可对同一切换重评，不设置业务关联时间窗口。使用切换 ID 与关联修订替换原结果，只有被验证的状态转变才改变贡献；重试不能重复增加样本。不得借重评重放已撤权、清除或过期的数据。

本次确认属于方案设计，不表示已开始读取本机聊天或进行模型调用。该规则应用于所有具有授权内容能力的来源/目标适配器，不只针对飞书与 Codex。
...
| 步骤 | 输入与输出 | 失败时行为 |
| --- | --- | --- |
| 1. 校验上下文 | 授权有效的业务事件、类型与证据、当前能力及真实选择门槛 | 缺样本先静默学习；主动查看可用固定入口 |
...
| 6. 个人偏好调整 | 校验真实样本门槛，对通过复核的候选做有限加权 | 冷启动仅学习，固定/最近入口仍可手动用 |
...
`next-action` 用途内使用闭合的三种操作 Schema：`classify-event` 返回事件类型、对象引用及证据；`attribute-transition` 返回已有事件 ID、切换 ID、来源与目标双方证据及修订、关联/无关/不确定状态；`recommend` 返回推荐。操作由 core 指定并在主进程白名单验证，不能混用 Schema。事件分类、归因同样需要真实模型、预算与版本检查；已验证的明确 ID 映射可直接复用，不必逐条切换调用模型。
...
学习期可以调用模型识别事件类型和归因已发生的真实选择，但不计算或弹出预测性建议；事件类型达到真实样本门槛后，才进入推荐调度。冷启动不是关闭语义分析，更不能靠模型生成虚构选择来凑样本。
...
**作用范围已由用户确认：通用习惯共享，项目特殊习惯优先。** `scope` 分为本人通用与当前项目两层。功能向导说明这一用途，默认将用户已同意用于学习的项目中的类型级选择积累为本人通用偏好；不跨用户或工作区账户共享。比如项目 A 的多次“修复反馈 → Codex”，可用于项目 B 的新修复反馈，不要求每个项目从零学习。

解析优先级为：本次明确指定的目标 → 当前项目的明确偏好 → 当前项目已有足够真实样本支持的特殊习惯 → 本人通用偏好。用户明确设置“项目 B 的修复用 Claude Code”后，B 优先 Claude Code，A 和其他项目仍按各自/通用习惯；项目偏好冲突或不确定时不强行套用通用偏好。撤销项目特例后，可恢复仍有效的通用偏好。

跨项目只复用“事情类型 → 工具”的统计和明确偏好，不带出其他项目聊天内容、会话 ID、目标链接或来源证据。在 B 推荐 Codex 时，具体会话必须从 B 的授权目标中找；找不到则只提供能力匹配的应用级入口。用户可关闭通用习惯共享，此时只用项目内样本和配置。删除某项目学习记录时同步撤销它对通用聚合的贡献，不能把同一个事件在项目和通用层重复计数。

**冷启动策略已由用户确认：先积累真实选择，再推荐。** 按 `(scope, eventType, sourceApp, actionType)` 独立记录“学习中/可推荐”；初始工程门槛为至少 5 个不同业务事件中的有效真实工具选择，且有可核验归因与稳定偏好，才开放这一类事件的主动建议。5 是待评测的起始参数，不能仅凑够次数就宣称高置信。本人通用层对某类已就绪时，新项目可以继承该类工具偏好，不需要重新凑满 5 次；项目内自动学出的特例仍需足够本项目证据。显式项目偏好立即影响已就绪类型的排序，不绕过全新事件类型的冷启动门槛。某类学会不等于其他类型也已学会；模型生成的假设、重复曝光、重复切换、由推荐引导的点击不能用来填充初始真实样本门槛。

学习期主界面不弹进度提示；设置和主动查看入口显示“正在学习你的工具选择”。没有被动观察授权时，可从用户主动打开的固定/最近工具和明确关联中积累，不为了冷启动扩大权限。删除、撤权、纠错令有效样本不足时，该类别回到学习中。

每个工具候选维护正负贡献，先验 `Beta(1,1)`；不要求跨 3 天，不按停留时长加权。同一事件反复切到 Codex 不得冒充多个独立样本。对习惯变化，按同类事件的新样本顺序衰减旧贡献，初始衰减系数 0.98/新有效样本，并保留最多最近 50 个有效事件样本；仅延长两次操作之间的时间不改变权重。用户明确固定或排除立即生效。

| 反馈 | 处理 |
| --- | --- |
| 用户明确选择工具/目标，动作成功或用户确认正确 | 正向贡献 +1，应用选择与精确目标分别记账 |
| 用户明确表示这次不相关 | 撤销本事件错误归因及其正向贡献，本事件不再推同动作；不泛化成同类工具差评 |
| 用户明确表示此类事件不适合该工具，或指定偏好工具 | 按确认的类型/范围记录负向贡献 +1 或显式偏好；替代选择经过验证后正向 +1 |
...
没有真实模型有效事件分类或推荐结果时，只显示标记清楚的固定/最近入口。已有模型但真实样本不足时同样只显示固定/最近入口。统计不能替代模型理解事情类型，窗口标题关键词不能直接生成确定习惯。清除数据后恢复冷启动，并证明来源级、事件级、项目级与本人通用偏好都无残留。

## 12. 目标注册与跳转执行

跳转能力统一分为 `open-object`（具体会话/页面/文档/项目）与 `open-app`（应用或已登记服务入口）。适配器负责声明能力并解析可信目标，领域层、提示 UI 与执行器按能力处理，不按软件名称写特例。推荐显示前确定本次实际能力和目标；例如只会打开飞书应用，就不能写“回到这条消息”。

用户确认后精确目标突然失效时，按失效处理，并提供明确标注的应用级替代入口；不能不改文案就静默切成另一动作，也不能为完成跳转擅自新建内容。替代动作仍经用户选择与目标验证。学习统计分别记录选对工具、定位目标和执行故障，适用于所有软件。
...
| `next_action_feedback` | 事件/切换＋推荐＋反馈版本唯一，原因、事件/类型/目标范围、可撤销版本 | 明确反馈与归因纠错，区分误判、选错目标和执行故障 |
...
| `apps/desktop/src/main/next-action/`（新增） | 独立提示窗口、可配置确认键租约、临时输入状态、观察器、能力探测、注册与执行器 |
...
| 用户处于编辑/输入法组合 | Tab 模式暂缓卡片与租约，停止输入后重验建议并显示完整倒计时；恢复输入则先释放 Tab，内容不被读取 |
...
事件学习回放必须覆盖：修复反馈与评审请求都来自飞书，但分别选择 Codex 与 GitHub；不同消息中的同类事件可复用工具偏好；同一消息重复切换不重复计数；只识别应用也可形成有依据的工具层样本；没有归因证据则保留未知。将同一有效事件序列的停留时间替换为 15 分钟、45 分钟、两小时或隔天，在授权和数据仍有效时，归因与偏好结果应相同。另测 0/4 个样本不主动推荐、达到门槛但偏好冲突仍不推荐、事件类型独立学习、纠错后样本不足退回学习中、推荐点击不能填充冷启动门槛；纠错入口过期后仍可找到、只改本次与只改目标不误伤其他偏好、撤销纠错不双计数。另测通用偏好从 A 迁移到 B、B 特例不改变 A、撤销 B 特例回落通用、同一事件双层去重、删除 A 的贡献重建通用偏好、关闭共享后恢复项目范围；具体会话不得跨项目。另测音乐打断、多事项交替、锁屏/重启和删除；不能以拉长等待导致样本失效。

跨软件契约测试必须覆盖：同一套事件分别指向 AI 工具、通讯软件、网页服务和文档/编辑器；精确目标存在、仅应用入口、应用未安装、确认前目标失效四种状态。断言实际打开层级与文案一致、替代动作重新确认、未出现创建/发送/执行副作用；不能只把 Codex 流程跑通就认为其他软件已完成。

双边关联验收纳入现有分层评测集：同项目同一具体问题、同词不同项目、同项目不同事情、目标内容未授权、片段不足、多个候选都相似、引用撤回、模型超时、补充内容后重评及纠错替换。必须证明不确定状态零学习贡献，双方内容总量受限，重评幂等，模型输出不能伪造来源依据。
...
| 高置信覆盖率 | 已满足真实样本门槛的可推荐场景中给出高置信结果的比例 | ≥60%；另报全量事件的学习就绪率和实际覆盖，不能靠缩小分母刷分 |
| 冷启动静默 | 未积累足够真实选择的事件类型不主动弹推荐 | 所有相关用例通过；模型高置信也不能越过门槛 |
...
| NX11 | 通用 open-object/open-app 目标契约；飞书/GitHub/本机会话/事项/文档适配 | NX01,02,04 | 多软件复用契约，精确/应用级文案匹配，不假定入口存在，账号项目隔离 |
| NX12 | 上下文候选召回与用户确认映射 | NX03,11 | 明确关系优先，歧义不串项目，不依赖自动归并 |
| NX13 | next-action 分类/双边内容归因/推荐三种模型操作、提示和输出验证 | NX02,12 | 双方授权与引用核验、输入总量受限、不确定零贡献、重评幂等；Schema 不混用，无模型不伪造结果 |
| NX14 | 共用模型优先队列、取消、缓存和预算 | NX13 | 聊天不被背景阻塞，过期结果拒绝，调用次数有界 |
| NX15 | Windows CLI 可执行入口与服务适配验证 | NX01,13 | exe/cmd、路径空格、取消、无工具执行的真实用例 |
| NX16 | 真实选择门槛、本人通用工具偏好、项目特例优先、项目内目标和可删除贡献 | NX03,04,12,13 | 通用就绪可迁移、B 特例不改 A、撤销回落、共享开关/删除重建、双层去重，冷启动与前缀评测 |
| NX17 | 多软件通用执行器、确认令牌、目标重验与结果观测 | NX05,11 | 注入拒绝、双击去重、不静默换动作、不创建/发送，精确/应用/未知结果区分 |
| NX18 | 独立非激活短提示、确认键设置/租约、点击确认、次级查看入口 | NX13,14,17 | 改键/冲突/仅点击、焦点不变、按键不双传、到期/崩溃释放、多屏 E2E |
| NX19 | 边框倒计时、滑走、Tab 输入避让/有限恢复、去重和免打扰 | NX05,18 | 停止输入后完整 3 秒、恢复输入先释放、IME/未知状态、桌宠关闭/无障碍验收 |
| NX20 | 设置向导、权限状态、能力说明、模型字段预览 | NX05,06,07,08,10,13 | 首次授权/拒绝/无模型也能完成流程 |
| NX21 | 最近建议、按本次/类型/目标分层纠错、固定/排除和全维度删除 | NX04,05,16,18 | 跳走后仍可纠错、执行失败不扣偏好、撤销不双计数、冷启动回退、无残留 |
...
- [ ] 飞书、GitHub、本机会话、事项、文档均有真实可用的基础目标动作；统一采用精确打开或应用级入口，所有软件均不自动新建内容、发送消息或开工。
- [ ] 模型通过已配置的真实服务完成意图分析；无模型时无伪装的 AI 结果。
- [ ] 推荐引用可追溯，跨账号/项目不串数据，未知候选无法执行。
- [ ] 双边归因仅使用已授权来源和目标相关内容，保留双方依据；不确定暂不学习，补充内容重评不重复计数。
- [ ] 独立左上角短提示可在主窗口最小化、桌宠关闭或无 Live2D 模型时正常工作。
- [ ] 提示默认 3 秒，细边框倒计时，到点先解除确认键再滑走；键位与视觉有效期一致。
- [ ] 确认键可自定义、恢复默认或仅点击，设置与卡片键帽一致，改键先释放旧键，冲突不静默替换。
- [ ] 用户无需先聚焦提示即可按设定键跳转，单次确认不双传、不连跳，消失/失效/崩溃后恢复原键行为；不支持的平台不显示虚假键帽。
- [ ] Tab 模式输入中不弹也不占键，停止输入后重验并给足展示时间；重新输入先释放，未知输入状态不强行占 Tab；平台/输入法/读屏/减少动画验收通过。
- [ ] 不自动发消息、输入文字、执行代码，不因推荐或应用停留完成事项。
- [ ] 学到同类事件的工具选择，能用于新的同类消息；仅改变停留时长不改变归因和偏好，工具层与具体目标层分开验收。
- [ ] 通用工具习惯能迁移到新项目，项目特殊习惯优先；项目 B 的具体会话不从 A 获取，撤销特例可恢复通用偏好。
- [ ] 各类事件先积累真实选择再主动推荐；样本不足、重复切换、模型自造选择和推荐点击不能绕过冷启动门槛。
- [ ] 推荐消失或已跳走仍可找到最近建议；本次不对、此类工具偏好、分类错误、会话错误和执行故障分别处理，明确纠错可撤销，忽略不算负反馈。
...
进度以真实代码、测试报告和平台验证为准。当前已实现核心闭环、v28 可撤销贡献、系统配对存储和定向测试，逐项证据见实施记录；尚未通过实机/分发/试用等完整验收的项目保持待验收。现有需求表中已关闭的 X05 可行性评估不用重新标成“已实现”，应将新功能作为独立需求关联 Issue #158。
