...
**冷启动策略已由用户确认：先积累真实选择，再推荐。** 按 `(scope, eventType, sourceApp, actionType)` 独立记录“学习中/可推荐”；初始工程门槛为至少 5 个不同业务事件中的有效真实工具选择，且有可核验归因与稳定偏好，才开放这一类事件的主动建议。5 是待评测的起始参数，不能仅凑够次数就宣称高置信。某类学会不等于其他类型也已学会；模型生成的假设、重复曝光、重复切换、由推荐引导的点击不能用来填充初始真实样本门槛。
...
没有真实模型有效事件分类或推荐结果时，只显示标记清楚的固定/最近入口。已有模型但真实样本不足时同样只显示固定/最近入口。统计不能替代模型理解事情类型，窗口标题关键词不能直接生成确定习惯。清除数据后恢复冷启动，并证明来源级、事件级、项目级与可选共享偏好都无残留。
...
事件学习回放必须覆盖：修复反馈与评审请求都来自飞书，但分别选择 Codex 与 GitHub；不同消息中的同类事件可复用工具偏好；同一消息重复切换不重复计数；只识别应用也可形成有依据的工具层样本；没有归因证据则保留未知。将同一有效事件序列的停留时间替换为 15 分钟、45 分钟、两小时或隔天，在授权和数据仍有效时，归因与偏好结果应相同。另测 0/4 个样本不主动推荐、达到门槛但偏好冲突仍不推荐、事件类型独立学习、纠错后样本不足退回学习中、推荐点击不能填充冷启动门槛；纠错入口过期后仍可找到、只改本次与只改目标不误伤其他偏好、撤销纠错不双计数。另测音乐打断、多事项交替、锁屏/重启、共享偏好开关和删除；不能以拉长等待导致样本失效。
...
| NX16 | 真实选择积累门槛、类型级工具偏好、项目内目标排序、可选共享和可删除贡献 | NX03,04,12,13 | 冷启动不弹、类型独立就绪、纠错/删除后回退，前缀评测，无重复计数或跨项目泄漏 |
