...
前台切换先去抖 300 ms；稳定展示与主动打开才更新当前上下文。一次行为片段最长 2 分钟，遇到明确新上下文、锁屏、用户暂停、授权变更或连续空闲超过 30 秒即结束。时长用单调时钟，跨进程重启不拼接；墙钟只供展示。

后续目标属于同一片段，仍需至少一个关联依据：相同来源对象、明确事项关系、同项目的确认映射、或本次用户明确选择。单纯“看完飞书就切到浏览器”只产生应用级关联，不能学成某个 PR 偏好。

同时出现多个项目时保留歧义，不用“最近项目”强行填充。每设备会话序号去重；迟到超过片段窗口的事件只记诊断或丢弃，不改写已确认反馈。由 BUGU 打开的目标打上 `initiatedBy=bugu`，与自然行为分开统计。
...
由 BUGU 引导的选择与自然行为分桶，每片段最多一次贡献。不能把“自己推荐→用户点开→应用在前台”算成三次成功，制造自我强化。
...
| `next_action_episodes` | 上下文、起止、项目、质量等级 | 同一行为片段 |
...
| 锁屏、休眠、跨日、时间回拨 | 结束片段，恢复后重新取上下文；时长基于单调时钟 |
...
| NX03 | 新建纯 TS 领域包、上下文/片段状态机 | NX02 | 时序回放、空闲/锁屏/迟到/重启测试 |
