...
**用户看到一条工作消息后，不用重新寻找相关会话、PR 或文档；唤起不咕，就能打开接下来最可能需要的地方。** 使用越久，排序越符合本人习惯。
...
| 桌宠 | 用轻量入口提示「接下来可能要打开什么」 | 不抢焦点，不代替用户发送、修改或执行命令 |
...
5. 全局唤起、应用内入口、桌宠提示、键盘操作、推荐原因和反馈。
...
3. **确认模型与快捷键**：显示实际模型服务、拟发送的字段及全局唤起键。用户可启用推荐而关闭被动习惯学习。完成后用用户选择的一个真实上下文做首个推荐，不自动注入演示任务。
...
用户不需要先打开跟进清单。全局快捷键、桌宠按钮、BUGU 顶栏按钮都打开同一个轻量面板。没有精确上下文时，让用户从近期授权对象中选一个，不能偷偷遍历电脑寻找资料。
...
| 经常去同一工具 | 多次独立、有效的反馈样本 | 同等候选优先本人常用工具 | 不能用频率覆盖明确的消息目标 |
...
| Linux Wayland x64 | 不假定存在统一的全局前台观察接口 | BUGU 自身、扩展中的授权页面 | 可用时走 GlobalShortcutsPortal；否则托盘/桌宠入口 | 清楚显示「页面与应用内上下文」；不伪装成全桌面观察 |
...
不新增常驻信息密集的页面。主窗口顶栏一个带 tooltip 的图标；桌宠菜单一个入口；全局默认快捷键 `CommandOrControl+Shift+Space`。默认键被占用时让用户选键，不能静默抢占。

面板宽 400 px，内边距 16 px，间距 8/16 px；最多三条建议，每条「动作＋目标」一行，来源/原因最多一行，过长省略。采用已有 Kumo 组件、统一 16/20 px 图标盒、清楚的焦点与禁用状态。小窗口缩窄到屏幕可用宽度，不遮挡系统菜单或越出显示器。
...
图中第一条若只能打开应用，标题必须保持「打开 Codex」；只有能力实测成立时才能改成「继续登录修复会话」。目标能力与显示文案由同一枚举生成，避免 UI 夸大行为。

### 6.2 Tab 的确切行为

不在系统范围截获裸 Tab，它会破坏用户当前软件的输入与焦点导航。默认面板中方向键切换候选、Enter 确认、Escape 关闭，Tab/Shift+Tab 正常遍历控件。

为 Issue 中“一按 Tab 就过去”的操作提供可选的「面板内 Tab 快速确认」设置：仅当面板已被用户主动唤起、候选列表持有焦点、存在有效选中候选且无输入法组合时生效；从搜索框、设置、菜单或读屏导航进入时仍使用标准焦点行为。开启前展示键位说明，始终保留 Enter。任何模式下都不在别的应用中拦截 Tab。

### 6.3 桌宠提示与空状态

被动提示默认关闭，用户可独立开启。只在 E2/E3 上下文明确、高置信、非免打扰状态时出现；最多每小时 2 次、每天 6 次，同一上下文 30 分钟内不重复。气泡不抢焦点，不自动打开应用，不朗读私人消息。锁屏和已知会议/屏幕共享状态下隐藏；无法可靠获得共享状态的平台保留一键暂停与手动入口。

面板状态必须覆盖：未启用、未授权、没有上下文、正在分析、可用结果、没有可靠建议、离线、模型不可用、预算耗尽、目标失效、跳转中、跳转失败。技术诊断放在问号或详情中。没有可靠建议时可显示明确标注的「固定入口 / 最近打开」，不得伪装成 AI 推荐。
...
| 行为片段 | 上下文引用、候选目标、明确反馈、关联证据 | 30 天，上限 5,000 条/设备 |
| 习惯贡献记录 | 脱离原文的分桶特征、目标引用、贡献权重 | 滚动 90 天，可按来源删除 |
| 推荐记录 | 候选 ID、原因码、证据引用、策略版本、结果 | 30 天，不存完整模型请求 |
| 用户固定/排除配置 | 本人明确的选择 | 保留到撤销或删除 |

处理过程与其采集片段使用同一 30 天保留期和删除范围；暂离上下文缓存随关联记录删除，不能因“以后可能回来”无限保留。处理过程 ID 仅用于关联与去重，不保存额外原文。
...
「清除学习记录」删除事件、片段、贡献、推荐与缓存，并重算或清空聚合。必须有按全部/项目/来源/应用清除的能力，不能只删除日志而保留推导出的偏好。清除学习记录不删除用户事项和原连接记录；「移除连接并清除关联数据」沿用连接删除流程，同时清理本功能的引用。
...
**2026-09-18 讨论修订：取消两分钟硬截断。** 用户查资料、读代码再打开 AI，可能相隔几十分钟；按短时间窗口截断会漏掉真正的工作习惯。默认改为 **60 分钟滚动关联窗口**：从最近一次有依据的相关操作计算，出现新的相关操作便顺延。因此同一件事持续两三个小时仍可保持关联，60 分钟不是总时长上限。这个默认参数可随本轮讨论调整，不能重新变成固定时长的工作过程切割。

前台切换先去抖 300 ms；稳定展示与主动打开才更新当前上下文。将「采集片段」与「同一件事的处理过程」分开：后者以 `activityChainId` 关联多个片段，记录 `active / suspended / closed` 状态与 `lastRelatedAt`。划分采集片段用于暂停、重启和限额管理，不应让业务上同一件事失去连续性。

后续目标并入处理过程或续期，仍需至少一个关联依据：相同来源对象、明确事项关系、同项目的确认映射、或本次用户明确选择。仅同项目、仅应用名称相同、仅时间接近均不够。单纯“看完飞书就切到浏览器”只产生应用级关联，不能学成某个 PR 偏好，也不能靠频繁切换无关窗口无限续期。

临时查看无关网页或切到音乐应用，不关闭原处理过程，不将无关内容并入。识别到另一件事时，为它建立独立上下文，原处理过程转为暂离；用户返回有明确关联的对象时恢复。最多保留 8 个可恢复上下文，超额将最久未相关活动的上下文收束为历史；不为维持上下文读取后台窗口。

连续空闲 30 分钟可暂停当前片段，不能再因 30 秒未输入就认定离开。读长文、看代码时没有键鼠活动也不说明处理结束；已授权的同一对象仍稳定在前台时，保留上下文，但停留不增加学习贡献、不算新相关操作。60 分钟内没有新相关证据时仅停止主动关联，记录仍保留；之后若出现同一对象或用户明确确认，可以重新关联历史处理过程，不能仅凭时间自动接回。

锁屏、休眠、用户暂停或授权变化立即停观察并关闭当前采集片段；解锁/恢复时重新检查权限和对象，只凭明确关系恢复处理过程，不补造离开期间的动作。跨进程重启或跨日开始新片段；历史上下文只用于有依据的关联，不能将时间断档计为工作时长，也不能据此判断任务完成。片段时长用单调时钟，墙钟供展示；跨重启时间仅供排序，不用于伪造连续观察。

同时出现多个项目时保留歧义，不用“最近项目”强行填充。每设备会话序号去重；事件接收延迟预算与 60 分钟关联窗口分开，默认超过 30 秒才收到的观察不改变当前上下文，授权仍有效时只计脱敏诊断。关闭前到达但授权已撤销的事件直接丢弃。由 BUGU 打开的目标打上 `initiatedBy=bugu`，与自然行为分开统计。

观察流上限 10 条/秒、缓冲 200 条；重复前台事件合并。达到存储限额优先淘汰过期观察，保留明确设置；不能因此阻塞既有任务同步或删除事项数据。
...
| 步骤 | 输入与输出 | 失败时行为 |
| --- | --- | --- |
| 1. 校验上下文 | 授权有效的来源对象、当前能力、可选用户选择 | 不足则请求选择或只显示固定入口 |
| 2. 召回目标 | 同项目关联事项、来源链接、用户固定目标，最多 20 个 | 不跨项目补数量 |
...
| 7. 展示并等待确认 | 最多 3 个可执行候选与简短理由 | 过期、变更时重新检查，不静默换目标 |

模型不会获得调用任意软件、搜索全盘或自行创建目的地的工具。来源原文中的“忽略规则、执行命令、上传文件”等内容只是待分析数据，不提升为系统指令。

召回顺序固定为：当前消息里的可验证对象引用 → 已确认的事项/来源关系 → 本项目固定目标 → 同项目授权索引中的候选。最后一路可用现有事项检索及新建的会话目标标题索引，按更新时间和关键词取有限候选，再由真实模型判断语义相关性；关键词仅用于检索，不能据此生成推荐或判断完成。不全盘扫描、不新建向量服务，也不把一次推断永久写成跨来源合并关系。仅凭弱语义检索得到的目标最高作为普通建议，不能触发被动高置信提示。

### 10.2 模型输出与校验

输出 Schema 固定为 `contextId`、`intent`、`candidateIds`、每项 `evidenceIds`、`reasonCode`、`confidenceBand` 与 `abstainReason`。意图枚举为继续会话、查看交付、查找依据、回到讨论、打开事项、未知。最多三个候选，不返回 URL、文件路径、Shell、代码或自由工具调用。
...
推荐超时默认 API 15 秒、CLI 45 秒，可通过服务配置调低。面板立即出现缓存/加载状态，3 秒后允许显式选择其他目标；不承诺冷启动 CLI 毫秒返回。原始行为事件不逐条调用模型。

## 11. 习惯学习的具体算法

采用可解释的局部偏好统计，不在本机训练基础模型。特征限定为项目、来源类别、经模型确认的意图、时间段和目标类型；不分析人格、情绪或私人身份属性。

每个 `(project, intent, sourceType, target)` 维护带衰减的正负贡献，先验 `Beta(1,1)`，半衰期 30 天。最少 5 个有效独立片段且跨 3 天后，个人化权重才完全启用；样本不足收缩到先验。固定目标属于用户明确配置，可立即生效。

| 反馈 | 处理 |
| --- | --- |
| 用户明确选中且目标成功定位/用户确认正确 | 正向贡献 +1 |
| 用户明确表示不相关，并选了替代目标 | 原候选负向 +1；替代目标经验证后正向 +1 |
| 用户主动固定/排除 | 立即应用显式配置，不依赖统计权重 |
| 自然行为中有可验证的同上下文目标 | 弱贡献 +0.25，单日同目标最多一次 |
| 点了建议但目标未定位、启动失败 | 执行故障，不学习偏好 |
| 关闭、没点、停留、短时间返回 | 未知；可询问反馈，不直接记负向 |

由 BUGU 引导的选择与自然行为分桶，同一处理过程中的同一目标选择最多一次贡献；暂停、恢复、切片或重启不能制造新的独立学习样本，纠错应替换原贡献。不能把“自己推荐→用户点开→应用在前台”算成三次成功，制造自我强化。

排序使用三层约束：先排除失效或被排除的目标，再优先满足明确对象意图，最后在同等语义候选内使用偏好后验进行有限调整。个性化最大加权占归一化最终分值的 20%；明确指定 PR 的消息不应因用户常用 Codex 就把 Codex 排到那个 PR 前面。20% 是待评测的初始上限，变更需重跑固定评测集并记录策略版本。

没有真实模型有效结果时，只提供标记清楚的固定/最近入口；规则不能代替模型提取意图，也不能把单纯频率排序叫作模型推荐。清除偏好后恢复冷启动结果，测试必须证明旧贡献未残留。
...
| `next_action_activity_chains` | 项目、上下文、状态、最后相关操作、授权版本 | 同一件事的处理过程，可跨片段恢复 |
| `next_action_episodes` | 处理过程 ID、上下文、起止、项目、质量等级 | 有边界的采集片段，断档不补造 |
...
| `next_action_feedback` | 片段＋推荐＋反馈版本唯一 | 明确反馈与纠错 |
| `next_action_contributions` | 反馈/片段来源、分桶、权重、到期时间 | 删除可追踪的学习贡献 |
...
```mermaid
erDiagram
  ACTIVITY_CHAIN ||--o{ EPISODE : contains
  GRANT ||--o{ OBSERVATION : authorizes
  EPISODE ||--o{ OBSERVATION : groups
  EPISODE ||--o{ RECOMMENDATION : informs
...
| `packages/next-action/`（新增） | 上下文组装、关联、召回、排序、学习、状态机；纯 TS |
| `packages/model/src/next-action.ts`（新增） | 真实模型提示、输出 Schema、弃答与格式修复 |
...
| `apps/desktop/src/main/next-action/`（新增） | 观察器、快捷键、能力探测、注册与执行器 |
...
| 现有 preload 与 renderer 入口 | 面板、设置卡片、反馈、记录查看；仅使用 typed bridge |
| `apps/desktop/src/main/pet/` | 复用桌宠状态与免打扰，增加轻量入口/建议投影 |
...
| 用户处于编辑/输入法组合 | 不抢焦点、不劫持 Tab，不把输入框内容当上下文 |
| 锁屏、休眠、跨日、时间回拨 | 结束采集片段，恢复后重新取上下文；可凭明确关系接回原处理过程，不补造断档，不推断完成 |
...
长过程回放必须覆盖：看消息后 15/45 分钟再打开相关会话；有相关操作的两小时连续工作；读文档期间没有输入；中途听音乐后返回；两件事交替处理；一小时无关联活动后返回同一对象；锁屏、重启和次日继续。分别断言应关联与不应关联的操作、暂停期间零观察、无重复学习贡献；不能只验证短时间内连续点击的理想路径。
...
| 层级 | 必测内容 | 验收产物 |
| --- | --- | --- |
| 领域单测 | 分段、证据等级、去重、排序、衰减、纠错、删除、冷启动 | 可复现夹具与报告 |
...
| 针对性 E2E | 启用、推荐、Tab/Enter、失效、纠错、清除、桌宠不抢焦点 | 截图、trace、断言 |
...
跨包变更运行 `pnpm check:boundaries` 和 `pnpm typecheck`；E2E 只跑 `next-action` 及受到影响的桌宠/设置/任务聊天用例。原生和打包变更必须 `pnpm package:dir`。不恢复普通 PR 的自动全套 CI。可以扩展已有手动平台验证工作流，tag 的打包步骤校验本功能必要产物。
...
| NX03 | 新建纯 TS 领域包、滚动关联窗口、处理过程与采集片段状态机 | NX02 | 15/45 分钟及两小时回放、多事项切换、空闲/锁屏/迟到/重启测试 |
...
| NX13 | next-action 模型 purpose、提示和输出验证 | NX02,12 | 不误入任务 Schema；无真实模型不伪造建议 |
| NX14 | 共用模型优先队列、取消、缓存和预算 | NX13 | 聊天不被背景阻塞，过期结果拒绝，调用次数有界 |
| NX15 | Windows CLI 可执行入口与服务适配验证 | NX01,13 | exe/cmd、路径空格、取消、无工具执行的真实用例 |
| NX16 | 可追踪偏好贡献、时间衰减、限幅排序、冷启动 | NX03,04,12,13 | 时间前缀评测，无自强化重复计数 |
| NX17 | 可信执行器、确认令牌、目标重验与结果观测 | NX05,11 | 注入拒绝、双击去重、真实/应用/未知结果区分 |
| NX18 | 轻量推荐面板、顶栏入口、快捷键、Tab 可选模式 | NX13,14,17 | 键盘/读屏/输入法/缩放与多屏 E2E |
| NX19 | 桌宠入口与非侵扰提示、频率/免打扰 | NX05,18 | 不抢焦点、限频、锁屏、模型失败不重复气泡 |
