...
test('internal settings save, restart, shortcuts, clear and IPC restrictions', async () => {
  ...
  try {
    ...
    await page.getByText('保留与清理', { exact: true }).click()
    await page.getByLabel('最近记录保留时间', { exact: true }).selectOption('7')
    await page.getByLabel('习惯贡献保留时间', { exact: true }).selectOption('60')
    ...
    expect(stored.ok && stored.data.settings).toMatchObject({ enabled: true, shortcut: 'click', durationMs: 5000, historyDays: 7, contributionDays: 60 })
    ...
    await page.getByText('保留与清理', { exact: true }).click()
    await expect(page.getByLabel('最近记录保留时间')).toHaveValue('7')
    await expect(page.getByLabel('习惯贡献保留时间')).toHaveValue('60')
