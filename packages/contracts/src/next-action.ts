...
export const nextActionSettingsSchema = {
  ...
  properties: {
    ...
    shareAcrossProjects: { type: 'boolean' },
    proactive: { type: 'boolean' },
    shortcut: { type: 'string', maxLength: 64 },
    durationMs: { enum: [3000, 5000, 8000] },
    historyDays: { enum: [7, 14, 30] },
    contributionDays: { enum: [0, 30, 60, 90] },
  },
} as const
...
export const nextEventSchema = {
  ...
  properties: {
    ...
    eventType: { enum: nextEventTypes },
    action: { enum: ['handle', 'review', 'research', 'reply'] },
    createdAt: {
      type: 'integer',
      minimum: 0,
      maximum: Number.MAX_SAFE_INTEGER,
...
export const nextChoiceSchema = {
  ...
  properties: {
    ...
    attribution: { enum: ['confirmed', 'model', 'unknown'] },
    confidence,
    createdAt: {
      type: 'integer',
      minimum: 0,
...
/** No message text, titles, URLs or model responses; only revocable learning lineage. */
...
export interface NextLearningData {
  contributions?: NextContribution[]
  events: NextEvent[]
  choices: NextChoice[]
  grants: NextGrant[]
  ...
  oneOf: [
    {
      ...
      additionalProperties: false,
      required: ['method'],
      properties: { method: { const: 'nextAction.status' } },
    },
    {
      type: 'object',
      additionalProperties: false,
      required: ['method', 'settings', 'expectedVersion'],
      properties: {
        method: { const: 'nextAction.configure' },
        settings: nextActionSettingsSchema,
    ...
    },
    {
      type: 'object',
      additionalProperties: false,
      required: ['method', 'expectedVersion'],
      properties: {
        method: { const: 'nextAction.clear' },
        expectedVersion: revision,
      },
    },
    {
      type: 'object',
      additionalProperties: false,
      required: ['method', 'choiceId', 'kind', 'expectedVersion'],
      properties: {
        method: { const: 'nextAction.feedback' },
        choiceId: id,
        kind: {
          enum: [
            'wrong-event',
    ...
    },
    {
      type: 'object',
      additionalProperties: false,
      required: ['method', 'feedbackId', 'expectedVersion'],
      properties: {
        method: { const: 'nextAction.undo' },
        feedbackId: id,
        expectedVersion: revision,
      },
    },
...
const validateSettings = ajv.compile<NextActionSettings>(
  nextActionSettingsSchema,
)
...
export function parseNextSettings(value: unknown): NextActionSettings {
  ...
  return { ...defaultNextActionSettings, ...structuredClone(value) }
}

export interface NextHintView {
  id: string
  label: string
  keycap: string | null
  durationMs: number
