import { nextActionOutputSchema, type NextModelInput, type NextActionOutput } from '@memo/contracts'
...
export async function analyzeNextAction(input: NextModelInput, transport: TaskModelTransport, signal: AbortSignal): Promise<NextActionOutput> {
  ...
  const content = await transport({ purpose: 'next-action', schema: nextActionOutputSchema, signal,
    messages: [
